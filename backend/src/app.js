...
app.post("/api/semesters", async (req, res) => {
  ...
  if (mid1Date && mid2Date) {
    ...
    if (m1 > m2) {
      return res.status(400).json({ message: "Mid1 date must be before Mid2 date" });
    }
  }
...
app.post("/api/holidays", async (req, res) => {
  ...
  if (!name || !startDate || !endDate) {
    return res
      .status(400)
      .json({ message: "All fields are required: name, startDate, endDate" });
  }
  ...
  for (const h of existing) {
    ...
    if (s <= aE && aS <= e) {
      return res
        .status(400)
        .json({ message: "Holiday overlaps with an existing official holiday" });
    }
  }
